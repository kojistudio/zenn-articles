...
:::message
この節は後で実装コードと照合して正確に書き直す予定です（要確認）
:::

開発しているウェアラブルでは、センサフュージョン（VQF）がセンサの生データからクォータニオンを計算し、キャリブレーション時点のクォータニオンを「基準」として保存しています。以降の角度は「基準からの相対変化量」として計算します。

回転行列を明示的に扱っているかどうかは実装コードを確認中ですが、少なくとも「センサの取り付け方向のずれをキャリブレーションで吸収する」という考え方は回転行列の概念そのものです。**キャリブレーションが必要な理由**を理解するためにも、回転行列を知っておく価値があります。
